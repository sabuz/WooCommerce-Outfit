...
function wc_outfit_post_like_button($post, $count = true) {
	...
	if ($count == true) {
		$content .= '<span class="count">' . wc_outfit_get_post_like_count($post) . '</span>';
	}
	$content .= '</div>';
...
function wc_outfit_outfit_share_button($post, $url = null) {
	...
	$content .= '<a href="http://pinterest.com/pin/create/button/?url=' . esc_url($url) . '&media=' . wc_outfit_post_thumb_by_id($post, 'product-thumb') . '&description=' . get_the_title($post) . '" target="_blank" class="fa fa-pinterest-p"></a>';
